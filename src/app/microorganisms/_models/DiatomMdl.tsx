...
export default function DiatomMdl({ detail = false }: { detail?: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    outerGroupRef.current.rotation.y = t * 0.2;
    outerGroupRef.current.rotation.x = t * 0.1;
    innerRef.current.rotation.y = -t * 0.4;
    outerGroupRef.current.scale.setScalar(1 + Math.sin(t * 0.5) * 0.02);
  });

  return (
    <group scale={detail ? 2 : 1.2}>
      {/* Outer Silica Shell and Silica patterns grouped together */}
